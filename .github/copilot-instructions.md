...
- `handlers/advance.mjs` is the shared launcher for every dispatched producer state (Research, Design, SecurityReview): it validates the requested transition and the source state's committed diff, dispatches the target workflow, and records canonical state. `handlers/human-plan-review.mjs` enters the HumanPlanReview state (label + record + wait). `handlers/shared.mjs` holds the reusable canonical-load, attempt counter, and `resolveTargetHeadSha` producer-output validation. `handlers/initialization.mjs` runs Initialization inline.
